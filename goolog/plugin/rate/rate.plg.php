...
function rate_afterpost($post)
{
	if(isGET('id') && isGET('exp') && (isValidEntry('post', $_GET['id']) || isValidEntry('reply', $_GET['id'])))
	{
		rate_saveRate($_GET['id'], $_GET['exp']);
	}
	return rate_show($post);
}
...
function rate_show($id)
{
	...
	else
	{
		foreach($rate as $exp => $score)
		{
			$out .= '<a href="view.php/post/' .$_GET['post']. '/id/' .$id. '/exp/' .$exp. '"><img alt="" src="plugin/rate/img/' .$exp. '.png"></a> x '. $score. ' / ';
		}
	}
	$out .= '</div>';
	return $out;
